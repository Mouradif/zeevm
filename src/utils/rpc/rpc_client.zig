...
pub fn getCode(self: *RPCClient, address: u160) ![]u8 {
    ...
    const length = @divTrunc(code_response.len - 2, 2);
    const code = try self.allocator.alloc(u8, length);
    Hex.parseStaticBuffer(code_response, length, code);
    return code;
}

pub fn getStorageAt(self: *RPCClient, address: u160, slot: u256) !u256 {
    var parsed_response = try self.makeAddressAndSlotRPCRequest("eth_getStorageAt", address, slot);
    defer parsed_response.deinit();
    return try Hex.parseUint(self.allocator, parsed_response.parsed.value.result, u256);
}
...
fn makeAddressAndSlotRPCRequest(self: *RPCClient, req: []const u8, address: u160, slot: u256) !RPCParsedResponse(RPCBasicResponse) {
    var response = ByteArray.init(self.allocator);
    defer response.deinit();
    const payload = try std.fmt.allocPrint(
        self.allocator,
        "{{\"id\":1,\"jsonrpc\":\"2.0\",\"method\":\"{s}\",\"params\":[\"0x{x:0>40}\",\"0x{x:0>64}\",\"latest\"]}}",
        .{ req, address, slot },
    );
    defer self.allocator.free(payload);
    const result = try self.http_client.fetch(.{
...
const rpc = "http://geth.metal:10544";

test "Utils::RPCClient: Get Block Number" {
...
test "Utils::RPCClient: Get Chain ID" {
    var rpc_client = try RPCClient.init(std.testing.allocator, rpc);
    defer rpc_client.deinit();
...
test "Utils::RPCClient: Get Balance" {
    var rpc_client = try RPCClient.init(std.testing.allocator, rpc);
    defer rpc_client.deinit();
...
test "Utils::RPCClient: Get Nonce" {
    var rpc_client = try RPCClient.init(std.testing.allocator, rpc);
    defer rpc_client.deinit();
...
test "Utils::RPCClient: Get Storage" {
    var rpc_client = try RPCClient.init(std.testing.allocator, rpc);
    defer rpc_client.deinit();

    const storage_a = try rpc_client.getStorageAt(0xe592427a0aece92de3edee1f18e0157c05861564, 0);
    const storage_b = try rpc_client.getStorageAt(0xe592427a0aece92de3edee1f18e0157c05861564, 1);

    try std.testing.expectEqual(0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff, storage_a);
    try std.testing.expectEqual(0, storage_b);
}
